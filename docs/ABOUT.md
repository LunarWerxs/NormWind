...
- **a new shorthand utility group (e.g. a new Tailwind property pair that collapses to one utility)** - on an engine with `canonicalizeCandidates(..., { collapse: true })` nothing is needed: lib/engine-collapse.mjs asks Tailwind first and the table is only the fallback. For older engines, add the group to the vendored table in lib/vendor/tailwind-classname-groups.mjs (ported from eslint-plugin-tailwindcss 3.18.3, since v4 dropped the static table for a live-engine query and exports no equivalent); the merge-safety check in lib/merge-safety.mjs applies automatically to any group listed there. anchors: `lib/vendor/tailwind-classname-groups.mjs`, `lib/shorthand-families.mjs`

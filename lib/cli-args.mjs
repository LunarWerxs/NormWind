...
const VALUE_FLAGS = new Set(["--baseline", "--diff-base", "--ignore", "--reporter", "--theme-css"]);
...
function parseArgs(argv) {
    ...
    return {
        ...
        baselinePath: valueFlags["--baseline"] || null,
        checkCanonical: flags.has("--check-canonical"),
        cleanupCanonicalFiles: flags.has("--cleanup-canonical-files"),
        // Kept as "" when given empty, so the caller can reject it instead of
        // running ungated.
        diffBase: valueFlags["--diff-base"] ?? null,
...
Flags:
  ...
  --diff-base <ref>           Report, and exit 1 for, only findings on lines
                              changed since the merge base of <ref> (e.g.
                              origin/main), plus files git does not track yet.
                              Findings elsewhere are counted on stderr, so a
                              repo can adopt the audit without fixing old
                              debt first. Needs git and a fetched <ref>.
  --allow-empty               Exit 0 instead of 2 when the given pattern(s)
                              match no lintable files.
  --baseline <file>           Adopt NormWind on a codebase with existing
                              findings: a JSON file of per-file finding
                              counts. A file may keep up to its count; any
                              new finding fails, and a count that drops
                              fails until the baseline is lowered. Paths are
                              relative to the directory normwinds runs in.
  --update-baseline           With --baseline, write the current counts to
                              the file. Creates it on first use; afterwards
                              it only lowers or removes counts and refuses
                              to raise one.

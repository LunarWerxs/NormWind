...
// Family merges from Tailwind's own collapse (lib/engine-collapse.mjs) for one
// variant/important group. `entries` are the group's { raw, utility } tokens in
// order and `makeTarget` turns a bare utility back into this group's raw form.
// Returns null when the engine has no answer for the group or one of its merges
// fails the render-safety proof, so the caller falls back to the vendored
// family table; otherwise every merge, each judged against the group the
// previous ones left. The engine proves a merge equal to its own sources; the proof here
// covers the rest of the class list, which the engine never saw (a sibling
// `mx-8` that `ml-2 mr-2` used to override starts winning after `mx-2`).
function planEngineCollapse(entries, makeTarget, mergeSafety) {
    ...
    for (const { target, sources } of merges) {
        ...
        if (!mergeIsRenderSafe(groupRaws, sourceRaws, targetRaw, hasSibling, mergeSafety)) {
            // The engine only offers the widest collapse (all four sides to
            // `border-4`); when a sibling makes that unsafe, the table's
            // narrower axis merges may still be sound, so it takes the group.
            return null;
        }
...
// Fix-side view of planEngineCollapse: the same plan, as token indices.
...
function findFamilyMerge(tokens, mergeSafety = null) {
    ...
    for (const groupIndices of groups.values()) {
        // An engine answer is authoritative for its group, including "nothing
        // to collapse"; the table only speaks for groups the engine has not
        // answered. The engine's merges come back all at once so the group
        // lands in its collapsed form in one step.
...
function detectFamilyShorthand(groupedTokens, filePath, line, column, found, mergeSafety = null) {
    for (const tokens of groupedTokens.values()) {
        // Same precedence as findFamilyMerge on the fix side: the engine's
        // answer, when there is one, replaces the table for this group.
        const planned = planEngineCollapse(
            tokens,

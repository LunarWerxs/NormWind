...
// A --baseline note (a count to lower) is always gated: it describes the
// whole file, not the line it is pinned to.
export function partitionFindingsByChangedLines(findings, lookup, resolvePath) {
    const changed = [];
    const unchanged = [];
    for (const finding of findings) {
        const gated = finding?.baselineNote === true || isChangedLine(lookup, resolvePath(finding), finding.line);
        (gated ? changed : unchanged).push(finding);
    }

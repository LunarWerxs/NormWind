// Tailwind's own utility collapse, added on top of the vendored group table.
//
...
// pairs the table never had) and moves forward with each Tailwind release. It
// does not collapse every set the table does, though, so the table keeps its
// findings and the engine only adds the ones the table misses (see
// planEngineCollapse in lib/shorthand.mjs). Engines without the option, and
// runs that cannot load Tailwind, get the table alone.
//
...
function computeCollapseMerges(designSystem, utilities) {
    ...
    // direction-dependent, leaves the engine's answer only partly usable, so
    // the engine adds nothing for the group and the table answers alone.

...
function lookupEngineCollapse(utilities, probe = null) {
    ...
    if (Array.isArray(probe?.pendingCollapses) && !probe.pendingCollapseKeys.has(key)) {
...
function computeCollapseMerges(designSystem, utilities) {
    ...
    for (const target of output) {
        if (input.has(target) || COMPOSITE_OWNED_TARGET.test(target)) {
            continue;
        }
        const sources = /\s/.test(target) ? null : attributeSources(collapse, target, pool);
        if (!sources || [target, ...sources].some((utility) => DIRECTIONAL_PROPERTY.test(cssOf(utility) ?? ""))) {
            unexplained = true;
            continue;
        }
...
function recordCollapsedOutput(utilities, merges, output) {
    ...
    if (key.length <= MAX_COLLAPSE_KEY_LENGTH && !CANONICAL_MEMO.has(key)) {
        rememberDynamicCacheEntry(key, NO_MERGES);
    }
}
...
async function resolvePendingCollapses(pending) {
    ...
    for (const utilities of pending) {
        ...
        if (!supported) {
            rememberDynamicCacheEntry(key, ENGINE_CANNOT_COLLAPSE);
            continue;
        }
        ...
        rememberDynamicCacheEntry(key, encodeMerges(merges));
